...
class Table {
	public:
		Table();
		void insert( const DataType & entry );
		void find( int key, bool& found, RecordType<DataType> & result ) const;
		int size() const;
		void print() const;
   		void erase( int key );
	private:
		int hash( int key ) const;
		void findPtr( int key, bool& found, DataType * & ptr ) const;
		DataType * table[CAPACITY];
		int used;
};
